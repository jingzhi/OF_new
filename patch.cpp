...
namespace OFC
{
  ...
  PatClass::PatClass(   //Class initializer overload with definition in header
...
#endif
#if (SELECTMODE==1)
void PatClass::OptimizeIter(const Eigen::Vector2f p_in_arg, const bool untilconv)//p_in is either zero or from coarse scale
#else
...
#endif  
{
  ...
  Eigen::MatrixXf dx;
  Eigen::MatrixXf dy;
  Eigen::MatrixXf pweight_init=pc->pweight;
  Eigen::MatrixXf pweight_prev=pc->pweight*1.5;
  ...
  //while (  ! (pc->hasconverged  || (untilconv == false &&( pc->cnt > oldcnt))  )) 
  {
    ...
    // Projection onto sd_images
		...
		//	}
		    ...
		    if(validCnt<op->novals*0.7 && validCnt>op->novals*0.3  && !pc->hasoptstarted_2nd){
			//if(false){
				//cout<<"here!"<<endl;
		    ...
		    else if (!pc->hasoptstarted_2nd){
                pc->hasconverged_2nd=1; 
		    }
		    if(pc->hasoptstarted_2nd){
                pc->cnt_2nd++;
                pc->delta_p_2nd[0] = (dxx_tmp.array() * pc->pdiff_2nd.array()*((1-pc->isInlier.array()).abs())).sum()+(pc->isInlier.array()*(pc->pdiff_2nd.array().sign()*sigma_1*dxx_tmp.array())).sum();
                pc->delta_p_2nd[1] = (dyy_tmp.array() * pc->pdiff_2nd.array()*((1-pc->isInlier.array()).abs())).sum()+(pc->isInlier.array()*(pc->pdiff_2nd.array().sign()*sigma_1*dyy_tmp.array())).sum();
                ...
                pweight_prev_2nd=pc->pweight_2nd;
                OptimizeComputeErrImg_2nd();      // check if patch(es) moved too far from starting location, if yes, stop iteration and reset to starting location
		    }
        }
		...
		{
           pc->delta_p[0] = (pc->isInlier.cwiseProduct(pc->pdiff).array()*dxx_tmp.array()).sum()+(dxx_tmp.array() * pc->pdiff.array().sign()*sigma*((1-pc->isInlier.array()).abs())).sum();
           pc->delta_p[1] = (pc->isInlier.cwiseProduct(pc->pdiff).array()*dyy_tmp.array()).sum()+(dyy_tmp.array() * pc->pdiff.array().sign()*sigma*((1-pc->isInlier.array()).abs())).sum();
		}
    #else
